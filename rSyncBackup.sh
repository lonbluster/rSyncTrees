...
if [[ -z "$rsbHOME" ]] || [[ ! -d "$rsbHOME" ]] || [[ ! -w "$rsbHOME" ]]; then 
	rsbHOME=$HOME/rSyncBackup ; 
	if [[ ! -d "$rsbHOME" ]]; then mkdir $rsbHOME ; fi
fi
...
#and is a valid path , create necessary subdirs and refresh previous vars, or show invalid message
				...
				echo STORAGE=$store >> /etc/.rsyncbackup;
				fi
			else echo -e "Input something or set /etc/.rsyncbackup. $YELLOW U&e!$RESET" && exit
			fi
...
if tty -s; then 
	...
	if [ -n "$1" ]; then echo -e "\r $U_LINED..rSyncBackup cmd Options..$RESET "
		...
		rp) echo -e "\r"
			...
			echo -e "\r"
			if [[ $sync_first =~ ^[Ss]$ ]]; then echo -e "\n$RED Stand by to update sync $RESET"; nohup rSyncBackup && wait ; echo "Sync Complete..beginning compression.." && sleep 3; fi
...
#check storage is writable, then show the PID, and sourced files 
	...
	echo "rSyncBackupPID:"$$
	echo "Inc/Exclusion and log files from and to ${rsbHOME}/*"
	if [ -f "${custom_config}" ]; then
...
HELPless=("This is the DOC for the bash rSyncBackup.sh script. - May 2019 -

@@@@@@@@@@This message will be disable when HELP=off in /etc/.rsyncbackup.
...
-----------Press Q. Q and let it go. -------
------Press Q and let it go, or Enter 2 or more times to backup.-----------
"
)
...
#-displaying Restore input prompt, with recent backups
	...
	recent=$(find $rsbHOME -name "Bk*"  -print0 | xargs -r -0 ls -1 -t | head -3)
	#grep -o "date-*-*.*"
...
stdINC="\n/home\n/etc\n/var\n/root\n/boot"
include_file=${rsbHOME}/INCbackup.txt
...
#######################################################
echo "-----##-------SCRIPT-rSyncBackup--------##------" >> "$log"
(
#cat "$_rsync_script" ;
echo "-----##-------VARIABILI--------##------" ;
echo "Using configuration files from dir" $rsbHOME
echo -e '\r' ; echo "#" ;
...
mailx -n -s 'rsyncBackup-'$(hostname) -a $log -S smtp=smtp://$smtp_server $forward_mail < $custom_config 
fi
mv "$log" $newlog
